...
#region Internal Helper Functions
			...
			foreach (string s in SearchPath) {
				if (Directory.Exists (s)) {
					foreach (string f in Directory.GetFiles (s, "*.dll")) {
                        string _f = f.Replace(s, "").Replace(Path.DirectorySeparatorChar.ToString(), "");
                        if (_f.Substring(0, _f.Length - 4) == name)
                        {
	                        return s + Path.DirectorySeparatorChar + _f;
                        }
					}
				}
			}
		...
		/// <param name="_info">The <see cref="ECX.Core.Module.ModuleInfo" /> object representing the module to resolve dependencies for.</param>
		/// <param name="checking">If set to true modules will not be loaded as we are only checking dependencies, otherwise they will be loaded.</param>
		...
		protected void OpResolve (DepNode node, List<string> parents, ModuleInfo info, bool checking, bool opt) {
			...
			if ((_op == DepOps.And) || (_op == DepOps.Opt) || (_op == DepOps.Or) || (_op == DepOps.Xor)) {
				...
				foreach (DepNode _child in node.Children) {
					...
					// will be identical.
					try {
						OpResolve (_child, parents, info, checking, _op == DepOps.Opt ? true : false);
						_results.Add (true);
				...
				DepConstraint _constraint = node.Constraint;
				
				foreach (string _parent in parents) {
					if (_parent == _constraint.Name) {
						throw new CircularDependencyException ($"{_parent} and {info.Name} have a circular dependency on each other");
					}
				}
				...
				ModuleLoader _loader = new ModuleLoader (SearchPath, Controller);
				...
				if (_op == DepOps.NotLoaded) {
					if (Controller.IsLoaded (_constraint.Name)) {
						...
						if (!checking) {
							Controller.UnloadModule (_constraint.Name);
						}
					}
				}
			}
