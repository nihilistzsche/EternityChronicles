...
namespace MDK.Master
{
	...
	public static class TypeExtensions
	{
		...
		public static object CreateGenericInstance(this Type type, Type[] typeArgs, object[] args) {
			...
			return genericTypeDefinition.MakeGenericType(typeArgs).GetConstructor(constructorTypes.ToArray())?.Invoke(args);
		}
	}
	...
	public class MDKMaster
	{
		...
		{
			var mdkTypes = from a in AppDomain.CurrentDomain.GetAssemblies() from t in a.GetTypes() where t.GetInterface("IMDKDataSchema") != null select t;
			
			foreach (var type in mdkTypes)
			{
				Databases.Add(type, (IMDKDatabase)(typeof(MDKDatabase<>).CreateGenericInstance(new[] { type }, new object[] { this })));
			}
		}
