...
namespace IronDragon.Runtime {
    ...
    public partial class DragonMethodTable : DragonFunction {
        private readonly List<DragonFunction> _functions;

        public DragonMethodTable(string name) : base(name, null, null, null) {
            _functions = new List<DragonFunction>();
        }

        internal List<DragonFunction> Functions => _functions;
        ...
        public void AddFunction(DragonFunction function)
        {
            ...
            {
                if (func.Name.Equals(function.Name) && func.Arguments.Count == function.Arguments.Count && !(function is DragonNativeFunction))
                {
                    funcToRemove = func;
                }
            });
        ...
        internal DragonFunction Resolve(List<FunctionArgument> args, bool exactMatch = false) {
            ...
            if (_functions.Count == 1 && !exactMatch) {
                return _functions[0];
            }
            ...
            if (q.Any()) {
                ...
                fq = _functions.Where(f => f is DragonNativeFunction)
        ...
        private bool CheckForMatch(DragonNativeFunction function, List<FunctionArgument> args) {
            if (function.Arguments.Count == args.Count) {
                ...
                args.ForEach(arg => {
                    ...
                    {
                        val = DragonNumber.Convert((DragonNumber)val);
                    }
                    innerArgs.Add(val);
                });
                var match = true;
        ...
        public override string ToString() {
            var sb = new StringBuilder(string.Format("[DragonMethodTable]: {0} ({1})", Name, _functions.Count));
            sb.AppendLine();
            _functions.ForEach(func => {
